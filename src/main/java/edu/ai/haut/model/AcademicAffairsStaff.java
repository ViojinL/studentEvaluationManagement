...
public class AcademicAffairsStaff extends User {
    ...
    public String getFullInfo() {
        return String.format("工号: %s, 姓名: %s, 性别: %s, 部门: %s, 职位: %s",
                           id, name, gender, department, position);
    }

    @Override
    public String toString() {
        return String.format("AcademicAffairsStaff{staffId='%s', name='%s', gender='%s', department='%s', position='%s'}",
                           id, name, gender, department, position);
    }
}
